...
SDL_Color hColor = {0xFF,0xFF,0xFF};
...
bool LTexture::loadFromRenderedText(std::string textureText, SDL_Color textColor)
{
    ...
    free();
    printf("%s\n", textureText);
    SDL_Surface* textSurface = TTF_RenderText_Blended(main_font, textureText.c_str(), textColor);
...
bool loadMedia()
{
    ...
    backgroundMusic[0] = Mix_LoadMUS( "background0.wav" );
    ...
    backgroundMusic[1] = Mix_LoadMUS( "background1.wav" );
...
int main(int argn, char* args[])
{
  ...
  char s[1001000],v[100][2002],ctemp[1000],char_temp,stemp[1000], tchar[100];
  int word_count = 0;
  int overflow_check=0,full_length,line_temp=0,indi_pos_temp,left_sign_check=0;
  int selected_copy_ini_x,selected_copy_ini_y,selected_copy_final_x,selected_copy_final_y=0,selected_copy_check=0;
   ...
   SDL_Rect screenImageRect = {(200*SCREEN_WIDTH)/2000, (100*SCREEN_HEIGHT)/1000, (800*SCREEN_WIDTH)/2000, (500*SCREEN_HEIGHT)/1000 };

   bool newMenuBool = false, viewMenuBool = false, preferenceMenuBool = false;
   bool screenImagebool = false;
...
// resume theke string e newa
            // printf("savedStringFile Before: %s\n",savedStringFile);
            // fgets(savedStringFile, sizeof(savedStringFile), resumeFile);
            // printf("savedStringFile after: %s\n",savedStringFile);
...
//      Menu related work
            if(e.type == SDL_MOUSEBUTTONDOWN){
              ...
              if(e.button.x<SCREEN_WIDTH-100 && e.button.y >90 )
              {

              current_line=((e.button.y-90)/50)+scroll_count-up_sign;
               ...
               if( (e.button.x>0)&&(e.button.x<(SCREEN_WIDTH/20))&&(e.button.y>0)&&(e.button.y<(25*SCREEN_HEIGHT)/1000) && newMenuBool==false){
                ...
                viewMenuBool = false;
                ...
                else if(newMenuBool==true && (e.button.x > (10*SCREEN_WIDTH)/2000 && e.button.x < (193*SCREEN_WIDTH)/2000 && e.button.y > (265*SCREEN_HEIGHT)/1000 && e.button.y < (316*SCREEN_HEIGHT)/1000)){
                  ...
                  FILE *resumeFile = fopen(date.c_str(),"w");
                  fprintf(resumeFile, "%s", savedStringFile);
                ...
                else if(preferenceMenuBool==true && (e.button.x > (243*SCREEN_WIDTH)/2000 && e.button.x < (432*SCREEN_WIDTH)/2000 && e.button.y > (147*SCREEN_HEIGHT)/1000 && e.button.y < (219*SCREEN_HEIGHT)/1000)){
                  konGaanBaaje++;
                  konGaanBaaje %= 2;
                  Mix_HaltMusic();
                ...
                else if(preferenceMenuBool==true && (e.button.x > (253*SCREEN_WIDTH)/2000 && e.button.x < (429*SCREEN_WIDTH)/2000 && e.button.y > (237*SCREEN_HEIGHT)/1000 && e.button.y < (307*SCREEN_HEIGHT)/1000)){
                  konGaanBaaje = -1;
                  Mix_HaltMusic();
                }

                else if(preferenceMenuBool==true && !screenImagebool && (e.button.x > (246*SCREEN_WIDTH)/2000 && e.button.x < (430*SCREEN_WIDTH)/2000 && e.button.y > (50*SCREEN_HEIGHT)/1000 && e.button.y < (128*SCREEN_HEIGHT)/1000)){
                //  HeaderInputFromUser();
                  screenImageTexture = loadTexture("screenImageTexture.png");
                        ...
                        //paste something
                                                ...
                                                else if( e.key.keysym.sym == SDLK_v && SDL_GetModState() & KMOD_CTRL )
                                                {
                                                  ...
                                                  else
                                                  {
                                                    ...
                                                    {
                                                      tchar[i]=inputText[current_line].c_str()[length-1-i];
                                                      inputText[current_line].pop_back();
                                                      tchar[i+1]=0;
                                                    }
                                                    inputText[current_line] = inputText[current_line]+SDL_GetClipboardText();
                                                    inputText[current_line] = inputText[current_line]+tchar;
                                                  }
                                                                   ...
                                                                   if(ctemp[count-1]!=' '&&ctemp[count-1]!=','&&ctemp[count-1]!='.'&&ctemp[count-1]!='!'&&ctemp[count-1]!='?'&&ctemp[count-1]!='-'&&ctemp[count-1]!=';')
                                                                                    {
                                                                                        ...
                                                                                        if(word_count<(count-1))
                                                                                        {
                                                                                            ...
                                                                                            {
                                                                                                tchar[j]=ctemp[count-1-word_count+1+j];
                                                                                                sh.pop_back();
                                                                                                ctemp[count-1-word_count+1+j]=0;

                                                                                            }

                                                                                                ctemp[count-1-word_count+1]=0;
                                                                                            tchar[word_count+1]=0;
